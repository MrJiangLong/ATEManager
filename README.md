...
**与出厂报告的联动**：RETEST / ROLLBACK / RESET 破坏完工态时，该 SN 的报告任务自动
**作废**（状态 `invalid`，产物从 MinIO 与本地清除——报告反映的测试数据已失效；已上传
MES 的副本无法撤回，由 MES 保留历史）。产品重新完工后，完工扫描自动生成全新报告；
也可对作废任务手动「重试」重新生成。SCRAP 不作废报告（保留报废前最后有效状态的记录）。
作废记录随保留期（90 天）一并清理。
