...
def create_process(payload: schemas.ProcessCreateIn, db: Session = Depends(get_db), user=Depends(require_admin)):
...
def update_process(
    process_id: str, payload: schemas.ProcessUpdateIn, db: Session = Depends(get_db), user=Depends(require_admin)
):
    row = get_or_404(db, models.Process, process_id, "process")
...
def delete_process(process_id: str, db: Session = Depends(get_db), user=Depends(require_admin)):
...
def create_model(payload: schemas.ProductModelCreateIn, db: Session = Depends(get_db), user=Depends(require_admin)):
...
def delete_model(product_model: str, db: Session = Depends(get_db), user=Depends(require_admin)):
...
def create_station(payload: schemas.StationCreateIn, db: Session = Depends(get_db), user=Depends(require_admin)):
...
def update_station(
    station_id: str, payload: schemas.StationUpdateIn, db: Session = Depends(get_db), user=Depends(require_admin)
):
...
def delete_station(station_id: str, db: Session = Depends(get_db), user=Depends(require_admin)):
