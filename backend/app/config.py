...
def _tz_info() -> tzinfo:
    ...
    except Exception:  # 无 tzdata 或名称非法
...
class Settings:
    ...
    # ---- 防呆规则 ----
    # 连续失败达到阈值 → LOCKED（工程锁定，需人工处置）
    FAIL_LIMIT: int = _as_int(os.getenv("FAIL_LIMIT"), 3)
    # 固件基线校验开关：关闭后仅告警不拦截（换线调试期使用）
    ...
    # 统计日界时区："今日""日趋势"按该时区的自然日切分（默认东八区）
